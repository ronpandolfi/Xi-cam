...
def test_all():
    ...
    pixel_size = (172.0E-06, 172.0E-06)

    # wavelenth in meters
    wavelength = 1.23984E-10

    img = fabio.open('AgB_00006.edf').data

    # d-spacing for Silver Behenate
    d_spacings = numpy.array([58.367, 29.1835, 19.45567, 14.59175, \
                              11.6734, 9.72783, 8.33814, 7.29587, 6.48522, 5.8367])

    sdd = .3
    center = [620.0, 1647.0]
    #pdb.set_trace()
    geometry = PyFAIGeometry(
        sdd,
        center[1] * pixel_size[1],
        center[0] * pixel_size[0],
        1e-6,
    ...
    print geometry.get_fit2d()

    fit_param = ['distance', 'rotation', 'tilt', 'center_x', 'center_y']
    fit_thread = FitThread(geometry, d_spacings, img, fit_param, 40)
    fit_thread.start()
    ...
    pylab.imshow(numpy.log(img), interpolation='none')
