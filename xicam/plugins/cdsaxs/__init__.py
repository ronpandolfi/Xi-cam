...
class CDSAXSWidget(QtGui.QTabWidget):
    ...
    def loadRAW(self, Phi_min=-45, Phi_max=45, Phi_step=1, Pitch = 100):
        ...
        '''
        grid_x, grid_z = np.mgrid[0:400, 0:400]
        grid_x = grid_x/400.*(max(qx_carto)-min(qx_carto))-min(qx_carto)
        grid_z = grid_z/400.*(max(qz_carto)-min(qz_carto))-min(qz_carto)
        self.img = interpolate.griddata(np.stack([qx_carto,qz_carto]).T,profiles,(grid_x,grid_z),method='cubic')
        '''
