...
class CDSAXSWidget(QtGui.QTabWidget):
    ...
    def loadRAW(self, Phi_min=-45, Phi_max=45, Phi_step=1):
        ...
        self.QxyiData = cdsaxs.generate_carto(profiles, nb_pixel, Phi_min, Phi_step, self.pixel_size, self.SDD,
                                              self.wavelength, center[0])
        ...
        self.q, self.Qxexp, self.Q__Z = cdsaxs.find_peaks(profile_carto, self.QxyiDatacor, self.wavelength, nb_pixel,
                                                          self.pixel_size, self.SDD)
    ...
    def fitting_test1(self, H=10, LL=20, Beta1=70, Num_trap=5, DW=0.1, I0=1, Bk=0):  # these are simp not fittingp
        ...
        if algorithm == 'cmaes':
            ...
            self.cmaes(sigma=100, ngen=50, popsize=100, mu=10, N=len(initiale_value), restarts=0, verbose=False,
                       tolhistfun=5e-5, ftarget=None)
        elif algorithm == 'mcmc':
            self.fix_fitness = self.fix_fitness_mcmc
            self.residual(np.zeros(len(initiale_value)), test='True')
            self.mcmc(N=len(initiale_value), sigma=100, nsteps=100, nwalkers=4, use_mh='MH', parallel=False, seed=None,
                      verbose=True)
...
class CDModelWidget(pg.PlotWidget):
    def __init__(self):
        ...
        for i, color in enumerate('gyrbcmkgyr'):
            ...
            self.orders1.append(
                self.plot([], pen=pg.mkPen(color, width=2, style=QtCore.Qt.DashLine), name='Order ' + str(i)))
