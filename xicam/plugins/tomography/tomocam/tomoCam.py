...
temp_backproj = back_project(temp_y, nufft_params)  #*nufft_scaling/2
        ...
        for i in range(num_slice/2):
          for iter_num in range(num_iter):
            ...
            x_recon[i] = x_recon[i] + (C * back_project(R * (temp_y - Ax), nufft_params))[
                pad_idx, pad_idx]  #nufft_scaling
...
def gpuMBIR(tomo, angles, center, input_params):
    ...
    slice_2 = slice(1, num_slice, 2)
    gdata = afnp.array(new_tomo[slice_1] + 1j * new_tomo[slice_2], dtype=afnp.complex64)
    gradient = afnp.zeros((num_slice / 2, sino['Ns_orig'], sino['Ns_orig']),
                          dtype=afnp.complex64)  # temp array to store the derivative of cost func
    z_recon = afnp.zeros((num_slice / 2, sino['Ns_orig'], sino['Ns_orig']),
                         dtype=afnp.complex64)  # Nesterov method variables
    t_nes = 1
