...
import pipeline.tomopynames as tp

# from PySide import QtCore, QtGui
from PySide import QtCore, QtGui
from PySide.QtUiTools import QUiLoader
from pipeline import reader
from pipeline.scriptmaker import write_tomopy_script
...
class Wizard(QtGui.QDialog):
    ...
    def __init__(self, fileexplorer, parent=None):
        ...
        self.ui = QUiLoader().load('gui/recon_setup.ui', self)
            ...
            if self.ui.output_lineEdit != '':
                ...
                if self.ui.run_comboBox.currentText() == 'Local':
                    self.sigExternalJob.emit('Reconstruction',
                                             ...
                                             jobs.run_local_script, ['python', script_path], {})
                    # proc = spew.jobs.run_local_script('python', script_path)
    ...
    def getReconOptions(self):
        if self.ui.input_lineEdit.text() == '':
            ...
            reconOptions = TomopyReconOptionsWindow(self, self.tomopy_options, self.dset_metadata)
    ...
    def getReconParameters(self):
        if self.ui.recon_comboBox.currentText() == 'tomopy':
            reconParams = TomopyParametersWindow(self,
                                                 deepcopy(self.tomopy_options),
    ...
    def getFileMetadata(self, file_name, location):
        if location == 'Local':
            message = None
            try:
                data = reader.read_als_832h5_metadata(file_name)
            except IOError:
    ...
    def getJobParameters(self):
        if self.ui.run_comboBox.currentText() == 'Local':
            jobOptions = LocalJobParametersWindow(self, self.job_params)
            if jobOptions.exec_():
...
class TomopyReconOptionsWindow(QtGui.QDialog):
    """
    Class that handles input of tomopy reconstruction options
    """

    def __init__(self, parent, options, dset_metadata):
        QtGui.QDialog.__init__(self)
        self.parent = parent
        self.options = options
        self.dset_medatata = dset_metadata
        self.ui = uic.loadUi('gui/tomopy_recon_options.ui', self)
    ...
    def setDefaultOptions(self):
        ...
        self.ui.slice_start_spinBox.setValue(self.parent.recon_indices[0][0])
        self.ui.slice_end_spinBox.setValue(self.parent.recon_indices[0][1])
        self.ui.slice_step_spinBox.setValue(self.parent.recon_indices[0][2])
        self.ui.proj_start_spinBox.setValue(self.parent.recon_indices[1][0])
        self.ui.proj_end_spinBox.setValue(self.parent.recon_indices[1][1])
        self.ui.proj_step_spinBox.setValue(self.parent.recon_indices[1][2])
...
class TomopyParametersWindow(QtGui.QDialog):
    ...
    """
    def __init__(self, parent, options, params):
        QtGui.QDialog.__init__(self)
        self.parent = parent
        self.options = options
        ...
        for i, (func, params) in enumerate(dict.iteritems()):
            ...
            for j, (param, values) in enumerate(params.iteritems()):
                ...
                if isinstance(values[0], list):
                    ...
                    spinBox.setMaximum(9999999999999)
...
class LocalJobParametersWindow(QtGui.QDialog):
    """
    Class that handles input of tomopy reconstruction options
    """
    ...
    def __init__(self, parent, parameters):
        QtGui.QDialog.__init__(self)
        self.parent = parent
        self.ui = uic.loadUi('gui/local_job.ui', self)
